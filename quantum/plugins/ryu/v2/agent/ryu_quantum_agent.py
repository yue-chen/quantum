...
class GREPortSet(object):
    ...
    def setup(self):
        ...
        for port in self.int_br.get_gre_ports():
            try:
                ...
                LOG.debug("unknown node id %d %s %s. deleting it.",
                          node.id, node.dpid, node.address)
    ...
    def _add_port(self, node):
        port_name = _gre_port_name(self.tunnel_ip, node.remote_ip)
        self.int_br.add_gre_port(port_name, self.tunnel_ip, node.remote_ip)
    ...
    def _del_port(self, port_name, ofport):
        ...
        self.api.delete_port(rest_nw_id.NW_ID_VPORT_GRE,
                             self.int_br.datapath_id, ofport)
        self.tunnel_api.delete_port(self.int_br.datapath_id, ofport)
...
class OVSQuantumOFPRyuAgent(object):
    ...
    def _setup_integration_br(self, root_helper, integ_br, tunnel_ip,
        ...
        ryu_rest_client = OFPClient(ofp_rest_api_addr)
        gt_client = GRETunnelClient(ofp_rest_api_addr)
